...
func main() {
	...
	copiedFile := must(fs.Copy(drivefs.FileID(sourceFormID), drivefs.FileID(destinationFolderID), filename))

	formsService := must(forms.NewService(ctx, option.WithHTTPClient(client)))
	form := must(formsService.Forms.Get(string(copiedFile.ID)).Do())
